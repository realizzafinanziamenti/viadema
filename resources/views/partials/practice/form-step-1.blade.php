    ...
    @if (!$customerPreselected)
        <div class="flex flex-col gap-1.5 col-span-2">
            <div class="flex items-center justify-between">
                <flux:label>
                    Cerca cliente censito
                </flux:label>

                <x-buttons.inline-action-button
                    label="Crea anagrafica nuovo cliente"
            ...
            <x-dropdown-select
                ...
                :selected="$practiceForm->customerId"
                searchable
                search="customerSearch"
                placeholder="Seleziona cliente"
                setFunction="setCustomer"
                :has-error="$errors->has('practiceForm.customerId')"
            />
    ...
    <div class="flex flex-col gap-6 col-span-2">
        ...
        <div class="flex flex-col gap-1.5">
            ...
            <x-upload-files-container
                ...
                :has-error="$errors->has('temporaryFiles.*')"
                multiple
            />
            ...
            <div class="text-xs text-gray-custom-4">
                ...
                <div>
                    - Formati accettati:
                    jpg, png, pdf, doc, docx, xls, xlsm, xlsx, csv
                </div>
            </div>
        </div>
        ...
        @if (!empty($practiceForm->attachments))
            <div class="flex flex-col gap-1.5">
                <flux:label class="font-semibold">
                    Allegati temporanei caricati
                    {{ count($practiceForm->attachments) }}/{{ count($practiceForm->attachments) }}
                </flux:label>

                @foreach ($practiceForm->attachments as $index => $attachment)
                    <x-display-preview-file
                        :attachment="$attachment"
                        :index="$index"
                    />
                @endforeach
...
<div class="flex items-center justify-end gap-x-3 mt-18">
    ...
    @else
        <a
            href="{{ route('lead.show', ['id' => $selectedCustomer->id]) }}"
            wire:navigate
        >
            <flux:button
                variant="primary"
